...
# =============================================================================
#    Created on Sun Aug 7 2:17:40 2019
#   
#    @author: Reed
#
# What's New:
#       Now a visual display for the queue. Queue value indicates where in 
#       the actual queue object it is and green color indicates that the task
#       has been completed
# 
#
# Bugs:
#       -If the thread encounters an error, it's done for. Kill the whole
#       program and start it up again.
#
#      -Worked out kinks where it wasn't detecting runs processed and 
#        runs queued properly, but it will still queue up a run that is in the
#        middle of being processed.
# 
# =============================================================================
...
class LXeDataManager(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in (StartPage, WaveformViewer, AlphaViewer, RunFitting):
...
#         user inputs
    ...
    def refresh_queue(self,new = None,completed = None):
        ...
        elif isinstance(new,(str,unicode)):
            parents = new.split('/')[3:]
            for i in range(len(parents)):
